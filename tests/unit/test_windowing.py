...
Pin the baseline behavior of ``src.ingestion.windowing.window_vitals``,
...
class TestAvpuLimitation:
    ...
    def test_vitals_window_validates_bounds(self):
        recs = [_rec("heart_rate", 70, ISO0)]
        window = window_vitals(recs, "PT-001", window_minutes=5)
        # Model-level ge/le constraints are honored.
        with pytest.raises(Exception):
            VitalSignsWindow(
                patient_id="PT-001",
                window_start=window.window_start,
                window_end=window.window_end,
                heart_rate=500,  # violates le=300
            )
